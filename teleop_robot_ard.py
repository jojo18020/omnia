...
import serial  # NEW: for Arduino serial
...
def init_arduino():
    ...
    Update SERIAL_PORT as per your system:
    ...
    try:
        print(f"[ARDUINO] Connecting on {SERIAL_PORT} @ {BAUD_RATE}...")
...
def send_cmd_to_arduino(cmd: str):
    ...
    try:
        ...
        # Optional: read any response line Arduino prints
...
def handle_teleop_message(pubsub_message):
    ...
    try:
        ...
        if cmd in ("forward", "backward", "left", "right"):
            ...
            print(f"[{now}] TELEOP ignoring unknown cmd='{cmd}'")
